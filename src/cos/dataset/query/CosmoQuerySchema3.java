...
public class CosmoQuerySchema3 extends CosmoQueryTime{
	...
	public void propertyFilter(String particleType,String family, String proper_name,
		...
		try {
			...
			Filter rowFilter = hbaseUtil.getSubStringFilter("=","-"+particleType);				
	...
	public HashMap<String,HashMap<Long,String>> changeTrend(String particleType, String[] particles, ArrayList time_series,String family,String column){
		...
		try {
			...
			for(int i=0;i<particles.length;i++){
				particles[i] = Common.reverseIndex(particles[i]); // change the particles
				Filter rowFilter = hbaseUtil.getBinaryFilter("=", particles[i]+"-"+particleType);	
			...
			for (Result result : rScanner) {							
				String key = Bytes.toString(result.getRow());
				String particle = key.substring(0,key.indexOf('-'));				
 ...
 **************************************************************/
	 ...
	 */
	public ArrayList<String> copGetUnique(final int particleType, final long s1, final long s2) {
	...
	public HashMap<String, HashMap<Long, String>> copChangeTrend(final String particleType, String[] particles, final ArrayList time_series,String family,String column){
		...
		try{		    	    
			...
			FilterList pFilter = new FilterList(FilterList.Operator.MUST_PASS_ONE);
			for(int i=0;i<particles.length;i++){
				particles[i] = Common.reverseIndex(particles[i]);
			...
			System.out.println("DEBUG: "+particles.toString());			
			
			final Scan scan = this.hbaseUtil.generateScan(rowRanges,filterList, new String[]{family},new String[]{column},time_series.size());		    
	    
			System.out.println("scan start & stop: "+Bytes.toString(scan.getStartRow())+"; "+particles[particles.length-1]+"-"+particleType);
