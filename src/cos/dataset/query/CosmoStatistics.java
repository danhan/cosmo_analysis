...
public class CosmoStatistics {
	...
	/*
	 * This function is to caculate the points based on the given point and distance
	 */
	public ArrayList getNearPoints(String filename, boolean header, double x, double y, double z, double distance){
		
		ArrayList nearPoints = new ArrayList();
		try{
			// get all particles from one file
			System.out.println("start doing statistics on "+filename);
			...
			try{
				...
				while(line != null){
					...
					String[] metrics = line.split(CosmoConstant.METRICS_DELIMETER);
					long pid = Long.parseLong(metrics[CosmoConstant.INDEX_PID]);
					double px = Double.parseDouble(metrics[CosmoConstant.INDEX_POS_X]);
					double py = Double.parseDouble(metrics[CosmoConstant.INDEX_POS_Y]);
					double pz = Double.parseDouble(metrics[CosmoConstant.INDEX_POS_Z]);
					
					double d = Math.sqrt(Math.pow(px - x, 2.0)+ Math.pow(py - y, 2.0)+ Math.pow(pz- z, 2.0));
					if(d<=distance){
						nearPoints.add(pid);
					}
				...
				System.out.println("end doing statistics on "+filename);
				System.out.println("Points whose distance between the given points is "+distance+" are:"+nearPoints.size());
				System.out.println(nearPoints.toString());
	...
	public static void main(String[] args) {
		String input = "./data/first/cosmo50cmb.256g2bwK.00024.star.out";
		...
		double z = 0.14541020;
		double distance = 0.4;
		stat.getNearPoints(input, true, x, y, z, distance);
