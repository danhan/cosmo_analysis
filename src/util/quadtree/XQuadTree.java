...
public class XQuadTree  {
    ...
    private int max_objets_per_node = 3;
    ...
    public XQuadTree(Rectangle rect,int num_per_nodes)
    {
        m_rect = rect;
    ...
    public XQuadTree(int x, int y, int width, int height,int num_per_nodes)
    {
    ...
    private void subDivide()
    {
        ...
        int bi_width = m_rect.width / 2;
        int bi_height = m_rect.height / 2;
        
        Point mid = new Point(m_rect.x + bi_width, m_rect.y + bi_height);

        m_tl_child = new XQuadTree(new Rectangle(m_rect.x, m_rect.y, bi_width, bi_height),this.max_objets_per_node);
        m_tr_child = new XQuadTree(new Rectangle(mid.x, m_rect.y,bi_width, bi_height),this.max_objets_per_node);
        m_bl_child = new XQuadTree(new Rectangle(m_rect.x, mid.y, bi_width, bi_height),this.max_objets_per_node);
        m_br_child = new XQuadTree(new Rectangle(mid.x, mid.y,bi_width, bi_height),this.max_objets_per_node);
        ...
        for (int i = 0; i < m_objects.size(); i++)
        {
            ...
            System.out.println("========"+destTree.index);
            if (destTree != this)
            {
    ...
    public void insert(XNode item)
    {
        ...
        if (!m_rect.intersects((item.getRect()))
        		&& !m_rect.contains(item.getRect().getLocation())){        	
        	return;
        }
            ...
            (this.m_tl_child == null && m_objects.size() + 1 <= this.max_objets_per_node))
        {
            ...
            this.addObject(item);
            System.out.println("insert.... object are "+this.m_objects.size());
        }
        else
        {
    ...
    public void GetObjects(Rectangle rect, List<XNode> results)
    {
