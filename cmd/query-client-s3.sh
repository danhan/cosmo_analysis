...
USAGE="USAGE: <isCoprocessor(1,0)> <query(1,2,3,4)> <input-argument-configured>"
...
chmod a+x ${MYLIB}
GOOGLELIB=${PWD}/../lib/google-collections-0.8.jar
...
#echo ${COMMONLIB}:${HBASELIB}:${HBASECONF}:${HADOOPLIB}:${HADOOPCONF}:${MYLIB}:${GOOGLELIB}
${JAVA_HOME}/bin/java -Xmx1500m -classpath ${COMMONLIB}:${HBASELIB}:${HBASECONF}:${HADOOPLIB}:${HADOOPCONF}:${MYLIB}:${GOOGLELIB} cos.dataset.experiment.QueryClient4Schema3 $*
